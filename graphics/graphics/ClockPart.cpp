...
void ClockPart::drawClockPart(GLuint drawmode)
{

	// Draw the vertices
	glBindBuffer(GL_ARRAY_BUFFER, partBufferObject);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);

	// Bind the Gear normals
	glEnableVertexAttribArray(2);
	glBindBuffer(GL_ARRAY_BUFFER, partNormals);
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

	// Bind the colours
	glBindBuffer(GL_ARRAY_BUFFER, partColours);
	...
	{
		glPointSize(3.f);
		glDrawArrays(GL_POINTS, 0, partvertices);
	}
	else
	{
...
GLuint ClockPart::makeClockPartVBO(GLuint numlat, GLuint numlong, glm::vec3 colour)
{
	GLuint i, j;
	...
	makeUnitClockPart(pVertices, numlats);
...
void ClockPart::makeUnitClockPart(GLfloat *pVertices, GLuint numlats)
{
